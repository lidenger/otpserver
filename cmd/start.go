...
// 注意：即使对KEY文件提供了保护并不意味KEY文件可以被泄露
// 由于代码是开源的所以是获取到的，所以KEY文件依然是高敏感的
...
func GenKeyFile() {
	...
	if err == nil {
		panic(fmt.Sprintf("系统启动KEY文件已存在:%s", keyFile))
	}
	err = os.WriteFile(keyFile, []byte(cipher), 0x777)
	if err != nil {
		panic(err)
	}
}
